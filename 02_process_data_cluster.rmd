...
get_summary(ssh_con = "p284074@peregrine.hpc.rug.nl",
            password = password,
            type = c("all"))
...
walk2(data, c("info", "noinfo"), function(x,y){
  ...
  a_fig = map2(x, names(x), function(df_list, name){
    title = glue::glue('Φ (number of peaks): {name}, simulation type: {y}, M evolving')
  ...
  cairo_pdf(file = glue('figs/fig_v4_{y}_param_trajectory.pdf'), width = 10, height = 3,
            onefile = TRUE)
  {
    print(a_fig)
  }
...
walk2(data, c("info", "noinfo"), function(x,y){
  ...
  cairo_pdf(file = glue('figs/fig_v4_{y}_follow_prob.pdf'), width = 10, height = 3,
            onefile = TRUE)
  {
    print(a_fig)
  }
